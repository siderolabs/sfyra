...
func TestMachineDeploymentReconcile(ctx context.Context, metalClient client.Client) TestFunc {
	return func(t *testing.T) {
		...
		err = retry.Constant(1*time.Minute, retry.WithUnits(100*time.Millisecond)).Retry(func() error {
			var md v1alpha3.MachineDeployment

			err = metalClient.Get(ctx, types.NamespacedName{Namespace: "default", Name: machineDeploymentName}, &md)
			if err != nil {
				return err
			}

			if machineDeployment.Status.UnavailableReplicas != replicas {
				return retry.ExpectedError(fmt.Errorf("expected %d unavailable replicas, got %d", replicas, md.Status.ReadyReplicas))
			}
