...
func TestServerClassAny(ctx context.Context, metalClient client.Client, vmSet *vm.Set) TestFunc {
	return func(t *testing.T) {
		...
		err = retry.Constant(10 * time.Second).Retry(func() error {
			if e := metalClient.Get(ctx, types.NamespacedName{Name: v1alpha1.ServerClassAny}, &serverClass); e != nil {
...
func TestServerClassCreate(ctx context.Context, metalClient client.Client, vmSet *vm.Set) TestFunc {
	return func(t *testing.T) {
		...
		require.NoError(t, retry.Constant(2*time.Minute, retry.WithUnits(10*time.Second)).Retry(func() error {
			if e := metalClient.Get(ctx, types.NamespacedName{Name: defaultServerClassName}, &serverClass); e != nil {
				return e
			}
		...
		actualUUIDs := serverClass.Status.ServersAvailable

		actualUUIDs = append(actualUUIDs, serverClass.Status.ServersInUse...)
...
func TestServerClassPatch(ctx context.Context, metalClient client.Client, cluster talos.Cluster, capiManager *capi.Manager) TestFunc {
	return func(t *testing.T) {
		...
		os.Setenv("CONTROL_PLANE_PORT", "11111")               //nolint:errcheck
		os.Setenv("CONTROL_PLANE_SERVERCLASS", "dummyservers") //nolint:errcheck
		os.Setenv("WORKER_SERVERCLASS", "dummyservers")        //nolint:errcheck
		//nolint:errcheck
		...
		require.NoError(t, retry.Constant(5*time.Minute, retry.WithUnits(10*time.Second)).Retry(func() error {
			req, _ := http.NewRequestWithContext(ctx, "GET", metadataEndpoint, nil) //nolint:errcheck
			client := &http.Client{}
			...
			metadataBytes, e = ioutil.ReadAll(response.Body)
			if e != nil {
				return e
			}
